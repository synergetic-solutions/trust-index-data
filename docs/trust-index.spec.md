...
| DEC-18 | **Extraction outcomes, exception report, carry-forward** (2026-09-15). Every extracted host records an `outcome` (ok, empty, gated, blocked, missing, unreachable, transient, parse-error); `publish` writes `exceptions.{json,md}` and `changes.{json,md}` beside the release; the quality gate fails a release whose transient-failure rate exceeds 25% (warns above 5%). A trust center the run could not read keeps the prior release's frameworks, marked `staleSince` + `missedRefreshes`, and drops them after 2 consecutive misses (implements §11). A trust center read and found empty or gated is never carried | The 2026-09-08 refresh had 1,200 extraction errors on 3,328 hosts (36%) that the release rendered as "no frameworks"; a live re-probe found ~half would read fine on retry. Carrying facts across a bad network day keeps fidelity; the cap keeps a dead trust center from living forever |
| DEC-19 | **Access-gated trust centers are a first-class state**: `access` ∈ public / partial / gated on every record where the vendor exposes a signal (Vanta `UNAUTHENTICATED`, Conveyor document `access_level`, SafeBase item `publicAccess`); shown on the profile as "access-gated trust center: frameworks not publicly visible as of {date}" and reported as a metric on the changes page. Gated tenants' frameworks are never carried forward | ~27% of empty Vanta tenants are request-access gated, a genuine market signal that a blank frameworks box misrepresents. Honest-framing rule (§1): we report what the tenant chose to make public |
| DEC-20 | **BAA information is a published-page observation, not a HIPAA statement** (2026-09-18). Each record may carry `baa` ∈ public / gated from a document, link or FAQ on the organization's own trust center naming a business associate agreement, plus `baaUrl`; both overridable. Rendered only where HIPAA is advertised or the segment is healthcare, with the neutral wording "no BAA information was found on the public trust center as of {date}; many vendors provide a BAA through sales". Aggregates say "publish BAA information", never "offer a BAA". No cross-referencing of incident or breach records (the breach-timeline idea was closed for this reason). Published-controls counts follow the same rule: "controls the organization chose to publish", never coverage | The absent state read as "refuses to sign a BAA" would be a false factual claim about a vendor; title matching has false negatives a vendor could resent (hence the correction path); BAA availability is not HIPAA compliance. Under these rules the signal is the same class of fact as a framework badge: what the organization chose to publish, dated, correctable |
...
| Trust page unreachable at refresh | Retain prior frameworks/subprocessors, mark `staleSince` + `missedRefreshes`; drop after 2 consecutive missed refreshes, listed in `exceptions.md` (DEC-18). Applies to unread outcomes only; empty and access-gated reads are current facts |
| Refresh-wide transient failure (rate limits, NAS network) | `extract` retries transient hosts at lower concurrency, the workflow retries once more before publish, and the quality gate fails the release above a 25% transient rate so a bad day never publishes as market churn |
...
- **TI-07** ✅ built 2026-08-31 — `classify` stage (two-level taxonomy, LLM-labelled, prior labels carried forward; default model claude-sonnet-5, method in pipeline `docs/segment-classification.md`). The site keeps a small hand-curated overlay that wins where both exist.
...
- **Data-quality gate** ✅ built (`quality` command, DEC-16), not a numbered ticket; extended with the extraction exception report and transient-rate thresholds (DEC-18).
- **Extraction outcomes + carry-forward + changes report** ✅ built 2026-09-15 (DEC-18/19): `outcome`/`access` per host, `exceptions.{json,md}` and `changes.{json,md}` per release, prior frameworks carried across unread refreshes.
